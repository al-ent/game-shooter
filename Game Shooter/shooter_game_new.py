...
img_bonus = "rocket.png"  # Bisa pakai gambar lain
...
class Player(GameSprite):
    ...
    def reset(self):
        if self.boost_end_time > time.get_ticks():
            for i in range(3):
                color = (0, 255, 255, 120)
                offset_x = i * 20
                draw.line(window, color, (self.rect.x - offset_x, self.rect.y + self.rect.height // 2), 
                          (self.rect.x - offset_x - 20, self.rect.y + self.rect.height // 2), 3)
                draw.line(window, color, (self.rect.x + self.rect.width + offset_x, self.rect.y + self.rect.height // 2), 
                          (self.rect.x + self.rect.width + offset_x + 20, self.rect.y + self.rect.height // 2), 3)
        window.blit(self.image, (self.rect.x, self.rect.y))
...
class Enemy(GameSprite):
    def __init__(self, player_image, player_x, player_y, size_x, size_y, player_speed, enemy_type=1):
        ...
        self.health = 1 if enemy_type == 1 else 4
...
def main_menu():
    ...
    while menu:
        ...
        draw_text(window, "SPACE SHOOTER", font1, (255, 255, 255), win_width // 2, 200)
        ...
        for e in event.get():
            ...
            if e.type == KEYDOWN:
                if e.key == K_r:
                    ...
                    bonus_enemies.empty()
                    for i in range(5):
                        monster = Enemy(img_enemy, randint(80, win_width - 80), -40, 80, 50, randint(1, 5), enemy_type=1)
...
spawn_timer = 0
bonus_timer = 0
bonus_interval = 300  # Setiap 300 frame
    ...
    if not finish:
        if not paused:
            ...
            if bonus_timer >= bonus_interval:
                ...
                bonus_fly_sound.play(-1)  # Mulai suara bonus looping

            # Stop bonus fly sound kalau tidak ada bonus enemy
